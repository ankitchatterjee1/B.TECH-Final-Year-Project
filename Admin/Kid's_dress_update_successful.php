...
$con = mysqli_connect("localhost","root","","shopping_cart");
   ...
   if(isset($_POST['Submit']))
   {
	  ...
	  if($file_name1 == ""){
		  $sql="UPDATE `kid's_dress` SET `name`=' $Name',`price`='$amount',`details`='$details' WHERE `id`='$id'";
		  if(mysqli_query($con,$sql)){
			  header("location:Kid's_dress_edit.php?msg=ok");
		  }
		 }
		 else{
			$sql1="SELECT `image` FROM `kid's_dress` WHERE `id`='$id'";
			$qry=mysqli_query($con,$sql1);
			$row=mysqli_fetch_array($qry);
             
	   		 $sql2="UPDATE `kid's_dress` SET `name`='$Name',`price`='$amount',`details`='$details',`image`='$new_file_name1' WHERE `id`='$id'";
